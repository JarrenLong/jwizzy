...
char g_szAppName[] = "JincS WYSIWYG Editor v1.3";
char g_szChild[] = "v1.3 Child Window";
HINSTANCE g_hInst;
   ...
   LPSTR lpszCmdParam, int nCmdShow)
{
   ...
   if(!RegisterClassEx(&WndClassEx))
   {
    ...
    g_hMainWindow = CreateWindowEx(WS_EX_APPWINDOW, g_szAppName,
      "JincS WYSIWYG Editor v1.3", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,
      112, 57, 800, 600,
      0, 0, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
   switch(Message)
   {
      case WM_CREATE:
      {
         ...
         SendMessage(g_hStatusBar, SB_SETTEXT, 2, (LPARAM)"Copyright(c)2004 JincS. Made in collaboration with TCForums.net.");
         ...
         tbb[4].idCommand = CM_EDIT_UNDO;
         
         tbb[5].iBitmap = STD_CUT;
         tbb[5].fsStyle = TBSTYLE_BUTTON;
         tbb[5].idCommand = CM_EDIT_CUT;

         tbb[6].iBitmap = STD_COPY;
         tbb[6].fsStyle = TBSTYLE_BUTTON;
         tbb[6].idCommand = CM_EDIT_COPY;

         tbb[7].iBitmap = STD_PASTE;
         tbb[7].fsStyle = TBSTYLE_BUTTON;
         tbb[7].idCommand = CM_EDIT_PASTE;

         tbb[8].fsStyle = TBSTYLE_SEP;
         
         tbb[9].iBitmap = STD_HELP;
         tbb[9].fsStyle = TBSTYLE_BUTTON;
         tbb[9].idCommand = CM_HELP;

         SendMessage(g_hToolBar, TB_ADDBUTTONS, 10, (LPARAM)&tbb);
